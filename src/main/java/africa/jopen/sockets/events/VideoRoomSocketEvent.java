        ...
        switch (requestType) {
            ...
            case "joinRoom" -> {
                if (!messageObject.has("password")) {
                    response = XUtils.buildJsonErrorResponse(Requests.SERVER_BAD_REQUEST,  Events.EVENT_TYPE,  Events.VALIDATION_ERROR_EVENT,"password is required", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
                if (!messageObject.has("clientID")) {
                    response = XUtils.buildJsonErrorResponse(Requests.SERVER_BAD_REQUEST,  Events.EVENT_TYPE,  Events.VALIDATION_ERROR_EVENT,"clientID is required", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
                ...
                if (roomModelOptional.isEmpty()) {
                    response = XUtils.buildJsonErrorResponse(Requests.SERVER_BAD_REQUEST,  Events.EVENT_TYPE,  Events.VALIDATION_ERROR_EVENT,"Invalid room ID!", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
                RoomModel roomModel = roomModelOptional.get();
                if (!roomModel.getPassword().equals(room.password())) {
                    response = XUtils.buildJsonErrorResponse(Requests.SERVER_BAD_REQUEST,  Events.EVENT_TYPE, "authentication","Room authentication failed! Access rejected.", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
            ...
            case "createRoom" -> {
                if (!messageObject.has("pin")) {
                    response = XUtils.buildJsonErrorResponse(Requests.SERVER_BAD_REQUEST,  Events.EVENT_TYPE,  Events.VALIDATION_ERROR_EVENT,"Pin is required", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
                if (!messageObject.has("password")) {
                ...
                if (!messageObject.has("creatorClientID")) {
                    response = XUtils.buildJsonErrorResponse(Requests.SERVER_BAD_REQUEST,  Events.EVENT_TYPE,  Events.VALIDATION_ERROR_EVENT,"creatorClientID is required", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
