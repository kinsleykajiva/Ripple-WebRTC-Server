        ...
        switch (requestType) {
            ...
            case "joinRoom" -> {
                ...
                if (!roomModel.getPassword().equals(room.password())) {
                    response = XUtils.buildJsonErrorResponse(400,  Events.EVENT_TYPE, "authentication","Room authentication failed! Access rejected.", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
            ...
            case "createRoom" -> {
                ...
                if (!messageObject.has("creatorClientID")) {
                    response = XUtils.buildJsonErrorResponse(400,  Events.EVENT_TYPE,  Events.VALIDATION_ERROR_EVENT,"creatorClientID is required", response);
                    broadcast(clientObject, response.toString());
                    return;
                }
                ...
                response = XUtils.buildJsonSuccessResponse(200,  Events.EVENT_TYPE, "createRoom","Room created successfully", response);
            }
            default -> {
                response.put("clientID", clientObject.getClientID());
                response = XUtils.buildJsonErrorResponse(400,  Events.EVENT_TYPE,  Events.VALIDATION_ERROR_EVENT,"Invalid request type for VIDEO_ROOM feature", response);
            }
        }
        broadcast(clientObject, response.toString());
    }
