...
````
or here another way to install janus-gateway
...
The above approaches will install the service to run based on the file `/etc/supervisor/conf.d/janus.conf` .The logs are saved in `/var/log/janus.err.log` and `/var/log/janus.out.log` .The configuration files are in `/opt/janus/etc/janus/` .The default port is 8088 and the default admin secret is `adminsecret` .The default admin port is 7088 .
